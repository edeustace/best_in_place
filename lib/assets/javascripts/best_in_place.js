...
BestInPlaceEditor.prototype = {
  ...
  activate : function() {
    ...
    //note: we have no way of detecting if the user has 
    //cancelled a file selection - so we need to keep the click bound.
    if( this.element.attr('data-type') != 'file' )
    {
  ...
  processImages : function()
  {
    if( this.element.attr('data-type') == 'file')
    {
      ...
      this.element.html('<img src="'+imgUrl+'"/>');
    }
  },
  ...
  "textarea" : {
   ...
   "file" : {
    activateForm : function() {
      ...
      var uid = this.objectName + '_' + this.attributeName; 
      var iframeUid = uid + '_iframe';
      var formSpanUid = uid + '_form_span';
      var fileInputUid = uid + '_file_input';
      var url = arguments[0];
      var output = '<form class="form_in_place" action="'+url+'.json" method="POST" target="'+uid+'" encoding="multipart/form-data" enctype="multipart/form-data" style="display:inline;">';
      output += '<input type="hidden" name="_method" value="put"/>';
      output += '<input id="'+fileInputUid+'" style="width: 0px; height: 0px; visibility: hidden;" type="file" name="'+ this.objectName + '[' +this.attributeName + ']" value="' + this.sanitizeValue(this.display_value) + '"';
      if (this.inner_class != null) {
        output += ' class="' + this.inner_class + '"';
      }
      output += '></form>';

      if( $("#" + formSpanUid).length == 0 )
      {
        $('body').append('<span id="'+ formSpanUid +'" style="visibility:hidden; position:absolute;">'+output+'</span>');
        $("#"+ formSpanUid ).find("form").bind('submit', {editor: this}, BestInPlaceEditor.forms.file.submitHandler);
        $("#" + fileInputUid).bind('change', {editor: this}, BestInPlaceEditor.forms.file.changeHandler);
      }

      if( $("#" + uid ).length == 0 )
      {
        $('body').append('<iframe id="'+iframeUid+'" style="visibility:hidden; width: 1px; height: 1px; position: absolute;" name="'+uid+'"></iframe>')
        $('#' + uid ).bind('load', {editor: this, iframeUid:iframeUid}, BestInPlaceEditor.forms.file.iframeLoadHandler );
      }
      //we don't add the form, we keep the image instead.
      //this.setHtmlAttributes();
      var delayedTrigger = function() { $("#" + fileInputUid).trigger("click"); };
      setTimeout( delayedTrigger, 100 );
    },

    iframeLoadHandler : function(event){
      var iFrameResult = $("#" + event.data.uid).contents().find('body').html();
      $('#' + event.data.iframeUid).remove();
      
      var regex = /(\{.*\})/g;
      var matches = iFrameResult.match(regex);

      var jsonString = matches[0];
      var  resultObject = $.parseJSON(jsonString);
      var html = '<img src="'+resultObject.display_as+'"/>'
      event.data.editor.element.html( html );   
      //for this data type - we never unbind so no need to rebind.
      //$(event.data.editor.activator).bind('click', {editor: event.data.editor}, event.data.editor.clickHandler);
    },
