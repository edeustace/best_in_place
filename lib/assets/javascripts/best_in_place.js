...
function BestInPlaceEditor(e, options ) {
...
BestInPlaceEditor.prototype = {
  ...
  activate : function() {
    ...
    //the file form behaves differently, as we have no way of knowing it has been cancelled.
    //in this case we keep the binding, and if the user cancels we do nothing.
  ...
  processImages : function()
  {
    ...
    {
      var url = this.element.html();
      var formName = this.objectName + "[" + this.attributeName + "]";
      var responseKey = this.element.attr('data-custom-response-key');
      var imageSpan = '<img class="bip_image" form-url="'+ this.url +'" ';
      imageSpan += 'href="'+url+'" ';
      imageSpan += 'form-name="'+formName+'" ';
      imageSpan += 'custom-response-key="'+ responseKey+'"></span>';
  ...
  {
    var inplaceOptions = {};
    if(this.options != null && this.options.imageOptions != null)
    {
      inplaceOptions = this.options.imageOptions;
    }

    inplaceOptions.data = {_method : "put"};

    var editor = this;

    inplaceOptions.onUploadComplete = function($element, resultText){
      console.log( "upload completed - dispatch ajax:success");
      editor.loadSuccessCallback.call(editor,resultText);
    };

    $(".bip_image").inplaceImageChanger(inplaceOptions);
  },
...
BestInPlaceEditor.forms = {
  ...
  "file" : {
    activateForm : function() {
      ...
      {
        var output = "<input style='visibility: hidden; width: 0px; height: 0px;' type='file' value='"+ file +"'></input>";
        this.element.append(output);
      }

      this.element.find("input").bind('change', {editor: this}, BestInPlaceEditor.forms.file.onChange);
      var editor = this;
      setTimeout( function(){
      
        editor.element.find("input").trigger('click');
      }, 200);
    },
    onChange : function(evt){
      ...
      reader.onloadend = function(evt){
        BestInPlaceEditor.forms.file.onLocalFileLoadEnd( evt, file, editor );
      } 
      reader.readAsBinaryString( file );
    },
    onLocalFileLoadEnd: function (evt, file, editor)
    {
      var url = editor.url;
      var name = editor.objectName + "[" + editor.attributeName + "]";
      var options = {
        onUploadComplete: function(responseText){
          //BestInPlaceEditor.forms.file.onUploadComplete.call( editor, responseText);
          editor.loadSuccessCallback.call( editor, responseText);
        },
        additionalData : {
          _method : "put"
        }
    ...
    onUploadComplete: function (responseText)
    {
      ...
      var key = this.element.attr("data-custom-response-key")
      this.element.html(obj[key]);
    }
...
jQuery.fn.best_in_place = function(options) {
  this.each(function(){
    if (!jQuery(this).data('bestInPlaceEditor')) {
      jQuery(this).data('bestInPlaceEditor', new BestInPlaceEditor(this, options));
    }
  });
  return this;
};
