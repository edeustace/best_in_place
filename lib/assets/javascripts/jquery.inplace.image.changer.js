...
com.ee.InplaceImageChanger.prototype = {
  ...
  _onLocalFileLoadEnd: function(evt, file)
  {
    ...
    builder = this.buildMultipartFormBody(file.name, file.type, evt.target.result, boundary);
  ...
  buildMultipartFormBody : function(filename, filetype, filedata, boundary) {
    var output = '';
    ...
    output += uploader.buildFileFormSegment( formName, filename, filetype, filedata, boundary);
  ...
  buildFileFormSegment : function( formName, fileName, fileType, binaryData, boundary)
  {
    var contentDisposition = 'Content-Disposition: form-data; name="'+ formName +'"; filename="' + fileName + '"';
    contentDisposition += this.crlf;
    contentDisposition += 'Content-Type: ' + fileType; 
